...
public class Filtre implements Filter {
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		try {
			if (filterConfig.getInitParameter("allowList") == null
					&& filterConfig.getInitParameter("denyList") == null
					&& filterConfig.getInitParameter("configLocation") == null) {
				useConfigLocation("classpath:filtre.properties");
			} else {
				addressManager.extendAllowList(filterConfig
						.getInitParameter("allowList"));
				addressManager.extendDenyList(filterConfig
						.getInitParameter("denyList"));
	...
	private void useConfigLocation(String configLocation)
		...
		addressManager.extendAllowList(configReader.getCompositeAllowList());
		addressManager.extendDenyList(configReader.getCompositeDenyList());
	}
